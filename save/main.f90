...
!  CALL volumeElementsOnSide((/5/),jj,jjs,sides,bndEl,iGlo)
!  
...
!  WRITE(20,*) ' n_rows = ' , SIZE(V_s%i)-1
!  WRITE(20,*) ' nnz    = ' , SIZE(V_s%j)
!  WRITE(20,*) '       i         j        e     '
...
!  
!  CALL start_matrix_P2bP1b ( jjs5 , X_s )
!  
!  CALL start_matrix_P2bP2belt ( jjs5 , jj5Belt , V_s)
!  
!  
!  !WRITE(*,*) "bndEl(3,:) = " , bndEl(3,:)
!  
!  ! CALL qs_b_0b0b_sp_M      ( m0s , jjs , alpha , AA )
!  CALL qs_b_0b0b_sp_M  ( bndEl(2,:) , jjs ,     1.0d0 , Mass_s )
!  
!  CALL qs_b_0b0bn_sp_M ( bndEl(2,:) , jjs , 1 , 1.0d0 , X_s )
!  
!  CALL qs_b_0b1n_sp_M  ( bndEl(2,:) , jj5Belt  , jjs5 , rr , 1 , 1 , 1.0d0 , V_s )
!  WRITE(*,*) " MINVAL(iGlo) , MAXVAL(iGlo) = " , MINVAL(iGlo) , MAXVAL(iGlo)
!  
!  
!  ! Test -----
!  OPEN(UNIT=20,FILE='./testRoutines/start_matrix_p2bp2belt.txt')
!  WRITE(20,*) ' n_rows = ' , SIZE(V_s%i)-1
!  WRITE(20,*) ' nnz    = ' , SIZE(V_s%j)
!  WRITE(20,*) '       i         j        e     '
!  DO i1 = 1 , SIZE(V_s%j)
!    WRITE(20,*) V_s%i_mumps(i1) , V_s%j(i1) , V_s%e(i1)
!  ENDDO
!  CLOSE(20)
!  ! ----------
!  
...
!  !
!  !   ! Test -----
!  !   OPEN(UNIT=20,FILE='./testRoutines/start_matrix_p2bp1b.txt')
!  !   WRITE(20,*) ' n_rows = ' , SIZE(X_s%i)-1
!  !   WRITE(20,*) ' nnz    = ' , SIZE(X_s%j)
!  !   WRITE(20,*) '       i         j       '
!  !   DO i1 = 1 , SIZE(X_s%j)
!  !     WRITE(20,*) X_s%i_mumps(i1) , X_s%j(i1) , X_s%e(i1)
!  !   ENDDO
!  !   CLOSE(20)
!  !   ! ----------
!  
...
WRITE(*,*)  "u,v(76915) = " , xx(76915) , xx(76915+np)
WRITE(*,*)  "u,v(79441) = " , xx(79441) , xx(79441+np)


!   ! Check ----
!   WRITE(*,*) " MAXVAL(ABS(xx(1:np))) = " , MAXVAL(ABS(xx(1:np)))
!   WRITE(*,*) " MAXVAL(ABS(xx(1+np:2*np))) = " , MAXVAL(ABS(xx(1+np:2*np)))
!   ! ----------


! Test wall stress computation --------------



   ALLOCATE(side_Id(1))
   side_Id(1) = 5
   CALL computeWallStress ( xx , rr , side_Id , tauW , force)
...
SUBROUTINE read_and_apply_boundary_conditions(input_file, k_d, rr, mms, jjs, &
   ...
   DO m = 1, number_of_sides
      ...
      READ  (21,*)  Dir(1,m),  Dir(2,m),  Norm_u(m),  Tang_u(m)
