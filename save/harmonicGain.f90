...
!
   ...
   USE extendedModule
   USE linearSystemEfficientSolution
...
!!!!!      CALL qc_00_zero_sp_boundary_cmplx (mms_In, jj, jjs, 1.0d0,  Qff_In) ! mms_In contains the boundary elements on the Inflow sides
!!!!!      WRITE(*,*) '   Matrix q00_Inflow built !'
!!!!!      
!!!!!      
...
!      CALL writeSparseMatrix ( Qf_In )
  ...
  ! build the matrix Qf_in (efficient way ? )!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     CALL qc_00_WallOnly_sp_boundary_cmplx  ( mmsInflow , jj , jjs , 1.0d0 , Qf_in ) 
...
!!!!!!! --------- CALL solveLinSystem( id ,  A , f  , ind , ud , u ) ----------------------------------------
!!!!!!      ind = ...
!!!!!!      ud  = ...
!!!!!!	    CALL solveLinSystem( DIRECT_SOLUTION , 6 , Wd , y1 , ind , ud , u )
!!!!!!! -----------------------------------------------------------------------------------------------------
...
! Create folders ex: './HarmonicGain/case/om' , './HarmonicGain/case/om/f' , './HarmonicGain/case/om/u'
            CALL system( 'mkdir -p '// TRIM(p_in%HG_output_directory) // TRIM(omegaString) )
            CALL system( 'mkdir -p '// TRIM(p_in%HG_output_directory) // TRIM(omegaString) //'/' // TRIM(p_in%HG_vol_in) // '/f' )
            CALL system( 'mkdir -p '// TRIM(p_in%HG_output_directory) // TRIM(omegaString) //'/' // TRIM(p_in%HG_vol_in) // '/u' )
