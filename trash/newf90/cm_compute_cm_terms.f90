...
! SUBROUTINEs :
!  computeCm      ( sig , mm , cm )
!  compute10terms ( ... ) TO BE WRITTEN
!  compute01terms ( x0,e0,      matIndTot,L,B,PhiC,PsiC,sigC,a,bb,QQ,GG)
!  computempterms ( x0,e0,order,matIndTot,L,B,PhiC,PsiC,sigC,a,bb,QQ,GG)
!  
!  !!! some inputs are unused ( e.g. sig in compute01terms() )!!!
!
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! ! ----------
    ...
    DO term = 1 , nterms
      ...
      ! g(:,nc+term) = 0 ;  already set by initialization to ZERO
        ...
        CALL writeComplex( cm , 'cm' , '(F12.8,A,F12.8,A)')
...
!  !     CALL writeComplex ( QQ(:,y) , 'QQ' , '(F8.5,A,F8.5,A)' , 8 )
      CALL writeComplex ( GG(:,y) , 'GG' , '(F12.8,A,F12.8,A)' , 8 )
      WRITE(*,*) "y       = " , y
!      WRITE(*,*) "GG(:,y) = " , GG(:,y)
      WRITE(*,*) "------------------------------------"
...
!  DO i1 = 1 , SIZE(A%i)+1
!  WRITE(*,*) i1,  A%i(i1) , eA%i(i1)
!  ENDDO
!  WRITE(*,*) "                 " , eA%i(SIZE(A%i)+1)
!  STOP
  nPhi = SIZE(Phi,2)
  nn   = SIZE(Phi,1)
...
! cm_compute_cm_terms +++++++++ check on matrix operations
! Check ----
!   IF ( imm(1) .EQ. 1 .AND. imm(2) .EQ. 0 .AND. ipp(1) .EQ. 1 ) THEN 
!   ALLOCATE(f(SIZE(QQ,1)))
!   CALL zAtimx( f , cm_K % e , &
!                    cm_K % j , &
!                    cm_K % i , &
!                    QQ(:,1) )
...
!   fPlot(:,1) = f
!   OPEN(UNIT=20,FILE="./plots/f_checkOut.txt")
!   DO i1 = 1 , np
