...
! WRITE(*,*) "numCmb = " , numCmb
   DO i2 = 1 , numCmb 
     ...
     CALL switchForcing (x0,e0,QQ(:,indV),i1,js_D,cm_Stiffness_cmplx,f_add)
     f = f + f_add
...
!     WRITE(*,*) " bb = " , bb(i1,:)
! ----
     IF ( a(i1) .EQ. 0 .AND. ANY(bb(i1,:).EQ.pp) ) THEN
       CALL a0bbForcing ( x0,e0,cm_Stiffness_cmplx,bb(i1,:), f_add )
       f = f + f_add   
     ENDIF
 ENDDO
