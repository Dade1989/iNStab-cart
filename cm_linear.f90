...
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! Module collecting subroutines to 
!  . start and compute the complex version of some useful matrices
!    (extended matrices; mass, stiffness, jacobian, identity)
!  . read the critical space from file
!  . direct and adjoint eigenvector normalization
! 
...
!
! SUBROUTINE :
!  start_extend_Jacobian ( jacobian , nPhi , ext_jacobian )
!  buildMassAndJacobian  ( x0,js_D_eigen, &
!                          Jacobian_cmplx,Mass_cmplx,cm_Stiffness_cmplx)
!  buildEyeMat           ( L , Eye_Cmplx )
!  readAndComputeCriticalSpace (...)
!  eigenvectorNormalization ( b , bj , bi , Phi , Psi )
!
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
SUBROUTINE buildMassAndJacobian (x0,js_D_eigen, &
  ...
  ! Stiffness Matrix
  Jacobian%e = 0.0d0
  CALL qc_11_sp_gg_M( mm , jj , 1.0d0 , Jacobian )

  ! impose boundary conditions on the Stiffness Matrix
!  CALL Dirichlet_rc_M (np, js_D_eigen, 0d0, Jacobian)
!  CALL Dirichlet_c_M (np, js_D_eigen, Jacobian)
  ...
  cm_Stiffness_cmplx%e       = CMPLX(Jacobian%e, 0.0d0, KIND=8)
...
SUBROUTINE readAndComputeCriticalSpace(PhiC,PsiC,sigC,nc)
  ...
  ALLOCATE(sigC(2))
  PhiC(:,1) = CMPLX(rePhi1,imPhi1,KIND=8) ; PhiC(:,2) = CMPLX(rePhi1,-imPhi1,KIND=8) 
  PsiC(:,1) = CMPLX(rePsi1,imPsi1,KIND=8) ; PsiC(:,2) = CMPLX(rePsi1,-imPsi1,KIND=8)
