...
SUBROUTINE collect (uu, pp,  xx)
   ...
   xx(2*np+1 : 2*np + np_L) = pp
 
END SUBROUTINE collect

!------------------------------------------------------------------------------

SUBROUTINE collect_Cmplx (uu, pp,  xx)
...
!   WRITE(*,*) " np , np_L = " , np , np_L
   
   IF ( ALLOCATED(xx) ) THEN ; DEALLOCATE(xx) ; ENDIF
   ALLOCATE(xx(2*np+np_L))
      
   xx(1     :  np     ) = uu(1,:)
   xx(1  +np:2*np     ) = uu(2,:)
   xx(1+2*np:2*np+np_l) = pp
...
SUBROUTINE extract_Cmplx (xx,  uu,  pp)
   ...
   !   np_L = SIZE(pp)
      ...
      ALLOCATE(pp(np_L))
      pp = xx(2*np+1 : 2*np + np_L) 
 
   ENDIF

!   COMPLEX(KIND=8), DIMENSION(:),   INTENT(IN)  :: xx
!   COMPLEX(KIND=8), DIMENSION(:,:), INTENT(OUT) :: uu
!   COMPLEX(KIND=8), DIMENSION(:),   INTENT(OUT), OPTIONAL :: pp
!   
!   INTEGER :: np, np_L
! 
!   np = SIZE(uu, 2)
...
SUBROUTINE extract (xx,  uu,  pp)

   IMPLICIT NONE
    
   REAL(KIND=8), DIMENSION(:),   INTENT(IN)  :: xx
   REAL(KIND=8), DIMENSION(:,:), INTENT(OUT) :: uu
   REAL(KIND=8), DIMENSION(:),   INTENT(OUT), OPTIONAL :: pp
