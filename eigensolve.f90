...
SUBROUTINE Read_eigenvalues (file_name,nev,eigenvalues)
...
!-----------------------------------------------------------
   ...
   IF (PRESENT(file_name)) THEN
      OPEN (UNIT = 19, FILE = TRIM(file_name), FORM = 'formatted', STATUS = 'unknown')
      WRITE(*,*) '--> Reading eigenvalues file: ' // trim(file_name) // ' ...'
   ELSE
      OPEN (UNIT = 19, FILE = 'eigenvectors', FORM = 'formatted', STATUS = 'unknown')
      WRITE(*,*) '--> Reading eigenvalues file: eigenvalues ...'
   END IF
   ...
   DO i = 1, nev
      ! good format for gnuplot and octave
      READ(19,*) real_s , imag_s
      eigenvalues(i) = CMPLX( real_s , imag_s , KIND=8 ) 
   END DO
   CLOSE(19)
...
!-----------------------------------------------------------
   ...
   ! output variables
   REAL(KIND=8) , DIMENSION(:) , ALLOCATABLE :: eigvctRe , eigvctIm
...
!  %--------------------------%
   ...
   structuralsens = SQRT(  ABS(u_lvect(1,:)*u_rvect(1,:))**2 &
                         ...
                         + ABS(u_lvect(2,:)*u_rvect(2,:))**2 ) / normalization
