...
!   WRITE(*,*) '    number_of_sides = ', number_of_sides
   ...
   DO k = 1, velCmpnnts
      CALL Dirichlet_nodes_gen (jjs, sides, Dir_tg(k,:),  js_D_tg(k)%DIL)
   ...
   !
   CALL Dirichlet_rc_M (np, js_D_tg, 0d0,  Mass)
...
! CREATE INITIAL GUESS
   ...
   SELECT CASE ( p_in%tranGrowth_initGuess )
      ...
      CASE (2)
         ...
         !CALL Dirichlet_c (np, js_D_tg, zero_bvs_D,  xx)
         CALL extract (xx,  uu)
...
! FILL MATRICES FOR THE TIME STEPPER
   ...
   SELECT CASE ( p_in%tranGrowth_method )
      ...
      CASE (2)
         ...
         CALL qc_11_sp_gg_M  (mm, jj,        1d0/Re,  Lns) ! stifness (GRAD:GRAD)
         CALL qc_oseen2_sp_M (mm, jj,         u0,     Lns) ! + linearized terms
         CALL qc_10_sp_M     (mm, jj, jj_L, -1d0,     Lns) ! + pressure gradient (ibp)
         CALL qc_01_sp_M     (mm, jj, jj_L, -2d0,     Lns) ! - velocity divergence
         CALL Dirichlet_rc_M (np, js_D_tg,   1d0,     Lns) ! Dirichlet BCs
         ...
         CALL qc_11_sp_gg_M  (mm, jj,        1d0/Re,  Lns) ! stifness (GRAD:GRAD)
         CALL qc_oseen2_sp_M (mm, jj,         u0,     Lns) ! + linearized terms
         CALL qc_10_sp_M     (mm, jj, jj_L, -2d0,     Lns) ! + pressure gradient (ibp)
         CALL qc_01_sp_M     (mm, jj, jj_L, -1d0,     Lns) ! - velocity divergence
         CALL Dirichlet_rc_M (np, js_D_tg,   1d0,     Lns) ! Dirichlet BCs
         IF (DESINGULARIZE_tg) THEN
            ! column
            WHERE (Lns%j == Nx)
...
!      ENDDO
   ...
   IF ( dirAdj == 1 ) THEN
      ...
      CALL Dirichlet_c (np, js_D_tg, zero_bvs_D_tg,  xx)
...
SUBROUTINE projectDiv (u_approx,  uDiv)
   ...
   CALL qc_00_zero_sp_M (mm, jj,        1d0,  prMatr) ! mass matrix
   CALL qc_10_sp_M      (mm, jj, jj_L, -1d0,  prMatr) ! + lambda gradient (ibp)
   CALL qc_01_sp_M      (mm, jj, jj_L, -1d0,  prMatr) ! - velocity divergence

   CALL Dirichlet_rc_M (np, js_D_tg, 1d0,  prMatr)
   IF (DESINGULARIZE_tg) THEN
   ...
   !
   tmpVel = 0d0
   CALL qv_00_sp (mm, jj, u_approx, 1d0,  tmpVel) ! alternatively CALL qv_mass_grad_sp (mm, jj, u_approx, 0*uu,  tmpVel)
   CALL collect (tmpVel, 0*p0,  rhsSol)
   CALL Dirichlet_c (np, js_D_tg, zero_bvs_D_tg,  rhsSol)
